...
Original code from SimpleDNS by mwarning
https://github.com/mwarning/SimpleDNS

Changed followings:
- Make A records slightly configurable
- Fix decode_domain_name to correctly handle domains with non-alphabet letters (digits, etc)
*/
...
{
  if (strcmp("networkattacker.com", domain_name) == 0) {
    addr[0] = 0x00;
    addr[1] = 0x00;
    addr[2] = 0x00;
    addr[3] = 0x00;
...
{
  char domain[256];
  for (int i = 0; i < MIN(256, len); i++) {
    uint8_t c = (*buf)[i];
    if (c == 0) {
      // This is the last fragment; erase trailing dot
      domain[i-1] = 0;
      *buf += i+1;
      return strdup(domain);
    } else {
      strncpy(domain+i, (char *)((*buf)+i+1), c);
      i += c;
      domain[i] = '.';
    }
  }
